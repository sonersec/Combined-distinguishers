...
cpaoutabsScaled = zeros(256, numpoint);

x1 = zeros(8,1);
x2 = zeros(8,1);
z = zeros(256, numpoint);
%calculating theoretical power trace for each trace and subkey
...
for counter = 1:32
    ...
    for numtraces=10:10:200
        ...
        %loglik    = @(maskedTraceVar,theta) sum(sum(maskedTraceVar.*log( b*(p*theta)' ) - b*(p*theta)'));
        ...
        for bnum = 5:5
            ...
            for kguess = 0:255
               ...
               meant = mean(mean(traces{counter}(1:numtraces, :)));
               ...
               for tnum = 1:numtraces
                   ...
                   tdiff = traces{counter}(tnum, :) - meant;
            ...
            for kguess = 0:255
                if maxcpa(kguess + 1) < max(maxcpa) * 0.2
                    cpaout(kguess+1,:) = 0;
                    cpaoutabs(kguess+1, :) = 0;
                end
                
                %need to define pdf of the distribution
                mu = mean(cpaoutabs(kguess+1, :));
                sigma = std(cpaoutabs(kguess+1, :));
            ...
            %update cpaoutabs
            for i = 1:numpoint
            cpaoutabsScaled(:,i) =  cpaoutabs(:,i) ./ transpose(phat);
            end
            
            weights = ones(1, numpoint);
            weights = weights .* mean(cpaoutabsScaled);
            
            %update traces
            
            %for i = 1:numtraces
            %    tracesnew(i,:) = traces{counter}(i,:) .* (weights*100);
            %end
            
            for i = 1:numtraces
                tracesnew(i,:) = (traces{counter}(i,:) -  transpose(mean(traces{counter}(i,:)))) .* (1+(weights*100));
            end
            ...
            for kguess= 0:255
                for point = 1:numpoint
                   z(kguess+1, point) = nmi(hyp{kguess+1}(1:numtraces, bnum), round((tracesnew(1:numtraces, point))*100));
                end
                
            end
            
            for i=1:256
               if max(z(i,:)) == max(max(transpose(z)))
                   if key{counter}(numtraces, bnum) == i-1
                    partialsuccess{counter}(bnum, iteration) = 1;
